...
```bash
gemini-toolbox -- -p "Run the tests for this Rust project"
```
...
```bash
gemini-toolbox -- -p "Spin up a Postgres container and a Redis container for testing"
```
...
```bash
cat /var/log/syslog | tail -n 50 | gemini-toolbox -- -p "Identify the root cause of these errors"
```
...
```bash
git diff --staged | gemini-toolbox -- -p "Write a semantic commit message for these changes"
```
...
gemini-toolbox --worktree --name refactor-auth -- -p "Refactor the authentication logic"
```
