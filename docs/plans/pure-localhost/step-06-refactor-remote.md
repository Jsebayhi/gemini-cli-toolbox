...
Having multiple ways to enable VPN (one for startup, one for on-demand) increases complexity and maintenance. By refactoring the existing `--remote` flag to use the sidecar model, we unify all networking into a single architecture. This allows us to remove Tailscale from the `gemini-cli` image entirely, reducing its size and attack surface while keeping the user experience identical.

## 📝 The "What" (Action Plan)
You must refactor the `--remote` flag in the wrapper script and clean up the CLI image's entrypoint.
...
- **Detached Mode:** If `-d` (detached) is used, simply launch both and exit.
