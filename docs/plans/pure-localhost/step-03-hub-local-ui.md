...
Update the Hub's HTML template and its main JavaScript logic to handle multiple connectivity endpoints.

## 📁 Files to Modify
- `images/gemini-hub/app/templates/index.html`
- `images/gemini-hub/app/static/js/main.js`
- `images/gemini-hub/app/static/css/style.css`

## 🛠️ The "How" (Technical Specification)

### 1. UI Badge (Template):
- In `index.html`, add a badge: `{% if m.local_url %}<span class="badge local">LOCAL</span>{% endif %}`.
...
- **Logic:**
    - If `local_url` is present, use it as the `href` for the "Open" button.
    - If ONLY a Tailscale IP is available, use the VPN URL.
    - **Advanced:** Implement a simple "Local Reachability" check. When the page loads, try to fetch a dummy resource (e.g., a 1x1 pixel) from `localhost`. If it fails (meaning the user is likely on mobile), hide the `LOCAL` badge and fallback to the VPN IP.
...
- **Verify:** Open the Hub from your desktop.
- **Verify:** Local sessions show the `LOCAL` badge and link to `http://localhost:XXXX`.
- **Verify:** Open the Hub from your phone (via VPN). The `LOCAL` badge should be hidden.
