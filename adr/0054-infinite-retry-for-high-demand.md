...
### Implementation
We delegate patching to a robust shell script (`images/gemini-cli/patch-cli.sh`) invoked during the build of both stable and preview images.
...
4. **Post-patch Verification:** The script tracks whether each patch (IDE and Retry) was applied at least once and fails the build if any of them were missed across all candidates.
...
- **Maintenance:** The shared script improves readability and testability of the patching logic.
